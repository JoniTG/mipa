...
<body dir="rtl">
  ...
  <main class="container blue_container">
    <form method="post" enctype="multipart/form-data" id="myform">
      ...
      <button type="button" class="button wide_button" name="con" id="con">מאוחר יותר</button>
    </form>
  </main>
  <script>
    $(document).ready(function() {
      ...
      $("#con").click(function() {
        $.ajax({
           method: "POST",
           url: "<?php echo ROOT_URL; ?>models/continue.php",
           data: { fullName: "<?php echo $_SESSION['fullName']; ?>", pass: "<?php echo $_SESSION['pass']; ?>",
           phone: "<?php echo $_SESSION['phone']; ?>", email: "<?php echo $_SESSION['email']; ?>" }
        }).done(function(resCon) {
          console.log(resCon);
           if(resCon == 1)
           {
             window.location.assign("<?php echo ROOT_URL; ?>user/");
           } else {
             alert("התרחשה שגיאה");
           }
        });
